    ...
    def execute(self, document, variable_values=None, timeout=None):
        ...
        try:
            ...
            try:
                ...
                print("Timeout error, re-executing the post request....!!")
                response = requests.post(self.url, **post_args)
            ...
            return ExecutionResult(errors=result.get('errors'), data=result.get('data'),
                                   extensions=dict(status_code=response.status_code, response_elapsed=response.elapsed))
        except Exception as e:
            print(f"Error executing a Graphql query: {e}")
            if e.response is not None:
                return ExecutionResult(
                    errors=[dict(reason=e.response.reason, text=e.response.text)], data=e.response.content,
                    extensions=dict(status_code=e.response.status_code, response_elapsed=e.response.elapsed)
                )
            else:
                return ExecutionResult(
                    errors=[dict(reason="No reason!!", text="No Text!!")],
                    data='No content!!',
                    extensions=dict(status_code=0, response_elapsed=-1)
                )
